                    ...
                    <div className="flex justify-between w-[100%]">
                        {
                            slicedCourses.map(course => {
                                return (
                                    <article className="w-[32%] bg-white px-[1.313rem] pt-[1.188rem] text-[black] rounded-[1.25rem] h-fit" key={course.id}>
                                        <Image className="w-[100%] h-[210px] object-cover rounded-[1.25rem]" src={course.image} alt={course.title} width={400} height={400} />

                                        <Link href={`/mycourses/id`}> <h3 className="font-medium text-[1.25rem] leading-[2.7rem] mt-[0.75rem] mb-[0.55rem]">{course.title}</h3></Link >
                                        <div className="flex items-center  mb-[1.563rem] ">
                                            <Image className="inline  bg-slate-500 h-[35px] w-[35px] object-cover object-top rounded-full" src={course.authorImg} alt={course.author} width={500} height={100} />
                                            <span className="font-medium text-[1.2rem] leading-[1.688rem] ml-[0.938rem]">{course.author}</span>
                                        </div>
